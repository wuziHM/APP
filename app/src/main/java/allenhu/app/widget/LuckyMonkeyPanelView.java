...
public class LuckyMonkeyPanelView extends FrameLayout {
    ...
    public void setName(String[] names) {
        if (names.length != itemViewArr.length) {
            return;
        }
        for (int i = 0; i < names.length; i++) {
            itemViewArr[i].setText(names[i]);
        }
    }
    ...
    private void showLucker() {
        DialogPopup dialogPopup = new DialogPopup(activity);
        dialogPopup.setContent("中奖用户:" + itemViewArr[currentIndex].getText());
